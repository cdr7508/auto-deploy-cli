...
async function deploy(config) {
  const { script, webDir, distPath, projectName, name, shell } = config;
  try {
    execBuild(script);
    ...
    await restartServe(shell);
    await deleteLocalZip(shell);
...
// 第六步，重启线上服务（如果配置了的话）
async function restartServe(shell) {
  if (shell) {
    try {
      console.log(`（6）${shell}`);
      const spinner = ora('正在重启中');
...
// 第六步，删除本地dist.zip包
async function deleteLocalZip(shell) {
  return new Promise((resolve, reject) => {
    console.log(`${ shell ? '（7）':'（6）'}开始删除本地zip包`);
